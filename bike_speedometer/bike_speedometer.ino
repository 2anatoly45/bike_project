...
int riding_hours = 0;
unsigned long int riding_time = 0;                  // Riding time during trip in milliseconds
unsigned long int rest_start = 0;                   // Beginning of break
unsigned long int rest_end = 0;                     // End of break
unsigned long int rest = 0;                         // Total duration of break (rest)
...
void loop() { 
  ...
  if (magnetState == HIGH && magnetPrev == 0 && !resting) {
    magnetPrev = 1;
    ++mag;
    ++contacts;
  }
  else if (magnetState == LOW && magnetPrev == 1 && !resting) {
    magnetPrev = 0;
    ++mag;
    ++contacts;
  }
  ...
  if (resting)                                         // [ms]
    riding_time = rest_start;
  else
    riding_time = millis() - rest;
  ...
  convert_time(riding_time, riding_seconds, riding_minutes, riding_hours);
  convert_time(millis(), seconds, minutes, hours);

  printer();
  lcd_printer();
}

// Breaks time from milliseconds into sec, min, and hrs
...
void printer() {   //    Serial.print(mag); Serial.print("   ");
  if (counter == 0) {
    Serial.print("S ");                     // 2
